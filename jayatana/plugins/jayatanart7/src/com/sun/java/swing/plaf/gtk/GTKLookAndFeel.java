...
public class GTKLookAndFeel extends SynthLookAndFeel {
    ...
    private void installPropertyChangeListeners() {
        if(!pclInstalled) {
            ...
            GTKLookAndFeel.WeakPCL pcl = new GTKLookAndFeel.WeakPCL(this, kit, "gnome.Net/ThemeName");
            kit.addPropertyChangeListener(pcl.getKey(), pcl);
            pcl = new GTKLookAndFeel.WeakPCL(this, kit, "gnome.Gtk/FontName");
            kit.addPropertyChangeListener(pcl.getKey(), pcl);
            pcl = new GTKLookAndFeel.WeakPCL(this, kit, "gnome.Xft/DPI");
            kit.addPropertyChangeListener(pcl.getKey(), pcl);
    ...
    private static void flushUnreferenced() {
        GTKLookAndFeel.WeakPCL pcl;

        while ((pcl = (GTKLookAndFeel.WeakPCL)queue.poll()) != null) {
            pcl.dispose();
        }
    }
    ...
    private static class GnomeLayoutStyle extends DefaultLayoutStyle {
        ...
        public int getPreferredGap(JComponent component1,
                JComponent component2, LayoutStyle.ComponentPlacement type, int position,
                Container parent) {
            // Checks args
